...
.. note::
  ...
  et ses pixels seront **redimensionés** en conséquence,
...
.. note::
  ...
  Ces commandes ne dessinent rien par elle-même,
...
* ``x`` et ``y`` indiquent le coin supérieur gauche du rectangle,
  ``w`` inique sa largeur, ``h`` indique sa hauteur.
...
Sa valeur est exrimée en pixels (mais peut être un nombre flottant).
